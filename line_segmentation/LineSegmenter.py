...
class LineSegmenter():
    ...
    def __separate_lines(self, label, min_interpdist, height):
    ...
    def segment_lines(self, img):
        ...
        for i in range(1, len(clusters)):
            ...
            height = self.__get_line_height(Si, thresholded, min_region, minx, miny)
            new_baseline = np.array(list(Si.keys()))
            new_baseline[:,1] -= minx 
            new_baseline[:,0] -= miny 
            dilated, dist_transform, watershed_segments = self.__separate_lines(thresholded, min_region, height)
            ...
            contour = self.__trace_contour(filtered_shape)
