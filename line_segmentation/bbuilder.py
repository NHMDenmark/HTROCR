...
def find_projections(p, points_within_circle, angle):
...
def select_closest_interpdist_within_circle(p, angle, points):
    ...
    for radius in rs:
        ...
        projection = find_projections(p, points_within_circle, angle)
        if projection != -1:
            break
    if projection == -1:
        return 120
...
def small_distance(S1, S2):
    for pk in S1.keys():
        for qk in S2.keys():
            if L2_norm(pk, qk) < 20:
                return True

def cluster_points(states, Bs):
    S0 = states.copy()
    P_star = [S0] 
    unconnected_ps = []
    for p in states:
        ...
        for nv in NVs:
            ...
            interpdist = L2_norm(p, q)
            # if not following the skeleton - skip
            if isSameOriented and interpdist < 20:
                nv_connectivity.append([q, compute_connectivity((p,q), Bs)])
    ...
    while i < n:
        ...
        wasMerged = False
        while j < n:
            if small_distance(P_star[i], P_star[j]):
                P_star[i] |= P_star[j]
                del P_star[j]
                n -= 1
                wasMerged = True
...
def build_baselines(img):
    ...
    states = {tuple(p): (angles[i], select_closest_interpdist_within_circle(p, angles[i], S)) for i, p in enumerate(S)}
    clusters = cluster_points(states, Bs)
